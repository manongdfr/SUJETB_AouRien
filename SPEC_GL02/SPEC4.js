  ...
  function simuleexam(tableauDeDonnees) {
    ...
    */
    ...
    for (let i = 0; i < tableauDeDonnees.length; i++) {
      ...
      tableauDeDonnees[i].questionText = tableauDeDonnees[
        i
      ...
      if (tableauDeDonnees[i].typeDeQuestion == "Question classique") {
        for (
          let index = 0;
          index < tableauDeDonnees[i].reponses.length;
          index++
        ) {
          ...
          if (reponse === "") {
            ...
            console.log(
              `${index + 1}. ${reponse.replace("~", "").replace("=", "")}`
            );
            if (reponse.includes("=")) {
        ...
        //TICKET 5 : DONNER DU CONTEXTE À LA QUESTION POSÉE
        console.log(`tag de la question : ${tableauDeDonnees[i]}`);
      ...
      //Question type texte à trou
        ...
        correctAnswers = tableauDeDonnees[i].reponses[0]
          .substring(1)
          .split("=")
          .map((answer) => answer.trim().toLowerCase()); // Supprime le signe égal, divise les réponses, supprime les espaces supplémentaires et convertit en minuscules
        console.log(
          tableauDeDonnees[i].reponses[0].substring(1).split("=", "#")
        );
        //TICKET 5 : DONNER DU CONTEXTE À LA QUESTION POSÉE
        console.log(`tag de la question : ${tableauDeDonnees[i]}`);
        let reponse = prompt("\nVeuillez entrer votre réponse :")
          .trim()
          .toLowerCase(); // Supprime les espaces supplémentaires et convertit en minuscules
      ...
      //question type relié les mots entre eux
        ...
        let associations = tableauDeDonnees[i].associations.filter(
        ...
        for (let j = 0; j < associations.length; j++) {
          console.log(
            `Quelle est la bonne association pour "${associations[j].left}" ?`
          );
          //TICKET 5 : DONNER DU CONTEXTE À LA QUESTION POSÉE
          console.log(`tag de la question : ${tableauDeDonnees[i]}`);
          // Mélanger les associations de droite
          let associationsDroites = shuffle(
            associations.map((association) => association.right)
          );
          ...
          let reponseUtilisateur = prompt(
            ...
            .toLowerCase();
          console.log(
            "Vous avez choisi : " + associationsDroites[reponseUtilisateur - 1]
          );

          // Vérifier si la réponse de l'utilisateur est correcte
          if (
            associationsDroites[reponseUtilisateur - 1] ===
            associations[j].right
          ) {
            console.log("Bonne réponse !");
          } else {
            console.log(
        ...
        if (!aFaitUneErreur) {
          console.log(
            "Vous avez répondu correctement à toutes les associations pour cette question."
          );
          bonneReponse++;
        } else {
          console.log(
            "Vous avez fait au moins une erreur pour cette question."
          );
          mauvaise_reponses = mauvaise_reponses + 1;
        }
      }
    ...
    console.log(
      `Note finale : ${bonneReponse} / ${bonneReponse + mauvaise_reponses}`
    );
  }
